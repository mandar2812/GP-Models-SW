...
%DIF ADD revised-chapter.tex   Fri Aug  4 15:18:09 2017
...
In this chapter, we give the reader an \DIFdelbegin \DIFdel{indepth }\DIFdelend \DIFaddbegin \DIFadd{in depth }\DIFaddend view into building of probabilistic forecasting models for geomagnetic time series using the \emph{Gaussian Process} methodology outlined in the previous chapters. We highlight design decisions and practical issues \DIFaddbegin \DIFadd{that }\DIFaddend must be addressed in order to use \emph{Gaussian Process} models for probabilistic prediction of a quantity of interest. As a \DIFdelbegin \DIFdel{paedadogical }\DIFdelend \DIFaddbegin \DIFadd{pedagogical }\DIFaddend example, we formulate, train and test a family of \emph{Gaussian Process} Auto-Regressive models for one hour ahead prediction of the $Dst$ geomagnetic index.  
...
The quantities $l$ in the RBF, and $b$ and $d$ in the polynomial kernel are known as \emph{\DIFdelbegin \DIFdel{hyper-parameters}\DIFdelend \DIFaddbegin \DIFadd{hyperparameters}\DIFaddend }. \DIFdelbegin \DIFdel{Hyper-parameters }\DIFdelend \DIFaddbegin \DIFadd{Hyperparameters }\DIFaddend give flexibility to a particular kernel structure, for example $d = 1, 2, 3, \cdots$ in the polynomial kernel represents linear, quadratic, cubic and higher order polynomials respectively. The process of assigning values to the \emph{\DIFdelbegin \DIFdel{hyper-parameters}\DIFdelend \DIFaddbegin \DIFadd{hyperparameters}\DIFaddend } is crucial in the model building process and is known as \emph{model selection}. 
