...
%DIF ADD revised-chapter.tex   Wed Aug 30 14:14:45 2017
...
\DIFdel{data set hosted by NASA on the }\DIFdelend \DIFaddbegin \DIFadd{Real time data of solar wind parameters are recorded by the }\DIFaddend \emph{\DIFdelbegin \DIFdel{Space Physics Data Facility}%DIFDELCMD < } %%%
\DIFdel{(SPDF).
}%DIFDELCMD < 

%DIFDELCMD < %%%
\section{\DIFdel{Data Source: OMNI}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdel{The }\emph{\DIFdel{OMNI}} %DIFAUXCMD
\DIFdel{data set is a collation of in-situ measurements taken by orbiting satellites and }\DIFdelend \DIFaddbegin \DIFadd{ACE}}
\DIFadd{and }\emph{\DIFadd{DISCOVR}} \DIFadd{missions. Geomagnetic indices are often recorded in
}\DIFaddend ground based measurements\DIFdelbegin \DIFdel{of geomagnetic indices. The data itself is represented as time averaged versions of the various quantities and is available at two frequencies, by hour and by minute. The quantities recorded can be broadly classified into two groups.
}%DIFDELCMD < 
...
\DIFdel{)}\DIFdelend . \DIFaddbegin \DIFadd{The the }\emph{\DIFadd{Space Physics Data Facility}}
\DIFadd{(SPDF) hosted by NASA is a gateway for accessing data from a variety
